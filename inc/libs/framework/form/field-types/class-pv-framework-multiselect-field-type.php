<?php
/**
 * @class          PV_Framework_Multiselect_Field_Type
 * @version        1.0
...
class PV_Framework_Multiselect_Field_Type extends PV_Framework_Form_Field_Type_Base {
	...
	protected function generateItem( $option, $value, $selection ) {
		...
		$label   = is_string( $option ) ? $option : ( !empty( $option[ 'label' ] ) ? $option[ 'label' ] : '' );
		$checked = isset( $selection[ $value ] ) && $selection[ $value ] == 'checked';
		$styles  = isset( $option[ 'css' ] ) ? (array) $option[ 'css' ] : array();
		if ( $bgImage ) {
