...
class PV_API_Server{
	...
	public function __construct( $path ) {
		...
		$handler_class = apply_filters( 'provip_api_default_response_handler', 'PV_API_JSON_Handler', $this->path, $this );
	...
	public function check_api_key() {
		...
		if ( empty( $this->params[ 'GET' ][ 'key' ] ) || ! is_string( $this->params[ 'GET' ][ 'key' ] ) ) {
			return new WP_Error( 'provip_api_key_missing', __( 'API Key is missing.', 'provip' ) );
		}

		$key = pvGetOption( 'api_key', '' );
		if ( empty( $key ) || $key !== $this->params[ 'GET' ][ 'key' ] ) {
			return new WP_Error( 'provip_invalid_api_key', __( 'Invalid API Key.', 'provip' ) );
		}
	...
	protected function error_to_array( $error ) {
		...
		foreach ( (array) $error->errors as $code => $messages ) {
			...
			else if( is_string( $messages ) || is_numeric( $messages) ){
				$extra[$code] = $messages;
			}
		}

		return array( 'errors' => $errors ) + $extra;
	}
	 ...
	 *
	 * @since 0.1.2
	 * @uses  PV_API_Server::dispatch()
	 */
	public function serve_request() {

		do_action( 'provip_api_server_before_serve', $this );

		$this->header( 'Content-Type', $this->handler->get_content_type(), true );

		// the API is enabled by default
		if ( ! apply_filters( 'provip_api_enabled', true, $this ) || ( 'no' === pvGetOption( 'enable_api', 'no' ) ) ) {

			$this->send_status( 404 );

			echo $this->handler->generate_response( array(
				'errors' => array(
					'code'    => 'provip_api_disabled',
					'message' => 'API is disabled on this site'
				)
			) );
		...
		$served = apply_filters( 'provip_api_serve_request', false, $result, $this );
	...
	public function get_routes() {
		...
		$endpoints = apply_filters( 'provip_api_endpoints', $endpoints );
	...
	public function dispatch() {
		...
		switch ( $this->method ) {
			...
			default:
				return new WP_Error( 'provip_api_unsupported_method', __( 'Unsupported request method', 'provip' ), array( 'status' => 400 ) );
		}
		...
		foreach ( $this->get_routes() as $route => $handlers ) {
			foreach ( $handlers as $handler ) {
				...
				if ( ! is_callable( $callback ) ) {
					return new WP_Error( 'provip_api_invalid_handler', __( 'The handler for the route is invalid', 'provip' ), array( 'status' => 500 ) );
				}
				...
				$args = apply_filters( 'provip_api_dispatch_args', $args, $callback );
		...
		return new WP_Error( 'provip_api_no_route', __( 'No route was found matching the URL and request method', 'provip' ), array( 'status' => 404 ) );
	}
	...
	protected function sort_callback_params( $callback, $provided ) {
		...
		foreach ( $wanted as $param ) {
			if ( isset( $provided[ $param->getName() ] ) ) {
				...
				return new WP_Error( 'provip_api_missing_callback_param', sprintf( __( 'Missing parameter %s', 'provip' ), $param->getName() ), array( 'status' => 400 ) );
			}
		}
			...
			foreach ( self::$method_map as $name => $bitmask ) {
				foreach ( $callbacks as $callback ) {
					...
					if ( strpos( $route, '<' ) === false ) {
						$data[ 'meta' ] = array(
							'self' => getProVipApiUrl( $route ),
						);
					}
				}
			}

			$available[ 'store' ][ 'routes' ][ $route ] = apply_filters( 'provip_api_endpoints_description', $data );
		}

		return apply_filters( 'provip_api_index', $available );
	}
	...
	public function add_pagination_headers( $query ) {
		...
		$this->header( 'X-PV-Total', $total );
		$this->header( 'X-PV-TotalPages', $total_pages );

		do_action( 'provip_api_pagination_headers', $this, $query );
	}
